...
struct GetLocationAccessView: View {
    ...
    var body: some View {
        VStack{
            Spacer()
            HStack{
                Text("Для корректной работы приложения требуется доступ к службам геолокации устройства. Это необходимо для определения вашего местоположения на карте, а также для поиска ближайших остановок.")
            ...
            VStack{
                ...
                Button(action: {
                    navigationStack.append(FirstLaunchSelectionView.needAccessToNotifications)
                }, label: {
                    HStack{
                        if(buttonWasTapped){
                            Text("Продолжить")
...
struct GetLocationView_Preview: PreviewProvider {
    
    @State static var path = NavigationPath()
    
    static var previews: some View {
        GetLocationAccessView(navigationStack: $path)
    }
}
